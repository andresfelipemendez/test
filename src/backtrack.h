...
bool search(std::vector<int>& state, std::vector<std::vector<int>> &solutions, const ListOfPairs& listOfPairs) {
...
std::vector<std::string> solveEmilPuzzle(std::string rawInput) {
    auto splitedInput = splitInput(std::move(rawInput));
    std::vector<std::string> solutions;
    for(const auto& input : splitedInput){
